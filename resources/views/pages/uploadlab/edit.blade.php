                        ...
                        <form enctype="multipart/form-data" class="form-group" method="POST" action="{{ route('upload.update',$uploadLab)}}" >
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control" name="test_date" value="{{old('test_date') ?? $uploadLab->test_date}}"/>
                                ...
                                <div class="col-md-6">
                                    ...
                                    @foreach (REPORT_STAGE as $key => $value )
                                        <option @if((old('report_stage') ?? $uploadLab->report_stage) == $key) selected @endif value="{{$key}}">{{$value}}</option>
                                    @endforeach
                                    @endif
                                    </select>
                                  ...
                                  @endif
                                   ...
                                   <div class="col-md-6">
                                    <label for="Name" class="form-label">Next Test Date<a
                                                    style="text-decoration: none;color:red">*</a></label>
                                    <input type="date" class="form-control" name="next_test_date" value="{{old('next_test_date') ?? $uploadLab->next_test_date}}"/>
                            ...
                            <br/>
                            <?php $report_type = old('report_type') ?? null ?>
                       <div id="upload_div">
                                @if($uploadLab->upload_lab_type && !$report_type)
                                @foreach ($uploadLab->upload_lab_type as $key => $uploadlab_type )
                                <input type="hidden" style="display: none" value="{{$uploadlab_type->id}}" name="ids[]" />
                                    
                            <div id="uploadlab_inputs_{{$key}}" class="card uploadlab">
                                <div class="card-body" style="height:30vh"> 
                                    <?php $id = $uploadlab_type->id ?>
                                         ...
                                         <select required name="report_type[]" id="select_report_type_{{$key}}" class="form-control" >
                                           <option value="">--- Select any one ---</option>
                                              ...
                                              @foreach (REPORT_TYPES as $key => $value )
                                                  <option @if($uploadlab_type->upload_type == $key) selected @endif value="{{ $key }}">{{ $value }}</option>
                                               @endforeach
                                               @endif   
                                          ...
                                          <input class="form-control" required type="number" id="value_{{$key}}" name="value[]" value="{{ $uploadlab_type->value }}"> 
                                      </div>
                                    
                                        <div class="col-md-4">
                                            <label class="form-label">Benchmark Value</label>
                                             <input class="form-control" type="text" id="comments_{{$key}}" name="comments[]" value="{{ $uploadlab_type->comments }}"/>
                                        </div>
                                         </div>
                                   </div>
                            </div>
                            ...
                            @endforeach
                            @elseif($report_type)
                              @foreach (old('report_type') as $k => $uploadlab_type )
                              <?php $report_type = old('report_type')[$k] ?? null ?>
                                       <?php $values = old('value')[$k] ?? null ?>
